...
namespace WinWordControl
{
    public partial class WinWordControl : UserControl
    {
        ...
        public void LoadDocument(string t_filename)
        {
            ...
            { //×¢Ïú
               // wd.CommandBars.AdaptiveMenus = false;
                ...
                // wd.NewDocument += new Microsoft.Office.Interop.Word.ApplicationEvents2_NewDocumentEventHandler(OnNewDoc);//ApplicationEvents2_NewDocumentEventHandler(OnNewDoc);
              ...
              //  wd.DocumentBeforePrint += new Microsoft.Office.Interop.Word.ApplicationEvents4_DocumentBeforePrintEventHandler();
            ...
            if (wordWnd != 0)
            {
                ...
                // Microsoft.Office.Core.CommandBars  commandBars = (Microsoft.Office.Core.CommandBars) wd.ActiveWindow.Application.CommandBars;
                ...
                for (int i = 1; i <= counter; i++)
                {
                    try
                    {
                        Microsoft.Office.Core.CommandBar cb = (Microsoft.Office.Core.CommandBar)wd.ActiveWindow.Application.CommandBars[i];
                        cb.Enabled = false;
                      
                       #region ÉèÖÃ²Ëµ¥²¿·ÖÆÁ±Î
                        //Microsoft.Office.Core.CommandBar cb = (Microsoft.Office.Core.CommandBar)wd.ActiveWindow.Application.CommandBars[i];
                        ...
                        //if (nm == "Standard")
                        //{
                        //    int count_control = cb.Controls.Count;
                        //    for (int j = 1; j <= count_control; j++)
                        //    {
                        //        // MessageBox.Show(j+""+wd.ActiveWindow.Application.CommandBars[i].Controls[j].ToString());
                        //        cb.Controls[j].Enabled = false;
                        //    }
                        //}


                        //if (nm == "Menu Bar" || nm == "Drawing" || nm == "Tables and Borders")
                        //{
                        //    cb.Enabled = false;
                        //}
                        //else
        ...
        public void RestoreWord()
        {
            try
            {
                CommandBars cbs = wd.ActiveWindow.Application.CommandBars;
                int counter = cbs.Count;
                for (int i = 0; i < counter; i++)
                {
                    try
                    {
                        CommandBar cb = wd.ActiveWindow.Application.CommandBars[i];
                        cb.Enabled = true;
                    }
                    catch
                    {
              ...
              //  CommandBars  cbs = wd.ActiveWindow.Application.CommandBars;
                ...
                for (int i = 1; i <= counter; i++)
                {
                    try
                    {
                        ...
                        CommandBar cb = wd.ActiveWindow.Application.CommandBars.ActiveMenuBar;//[i];
                        String nm = cb.Name;
                        if (nm == "Standard")
                        {
                            int count_control = cb.Controls.Count;// ActiveWindow.Application.CommandBars[i].Controls.Count;
                            for (int j = 1; j <= 2; j++)
                            {
                                cb.Controls[j].Enabled = true;
                            }
                        }
                        if (nm == "Menu Bar")
                        {
