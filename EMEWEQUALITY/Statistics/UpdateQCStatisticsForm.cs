...
{
    public partial class UpdateQCStatisticsForm : Form
    {
        public UpdateQCStatisticsForm()
        {
            InitializeComponent();
        }
        ...
        /// 车辆表  QCInfo
        /// </summary>
        Expression<Func<View_QCRecordInfo_Update, bool>> expr = null;
        /// <summary>
        /// 实例化分页控件
        /// </summary>
        ...
        /// 排序字段及方式[以空格格开]
        /// </summary>
        private string orderbywhere = "QCInfo_UPDATE_TIME desc";
        /// <summary>
        /// 修改质检数据统计的加载显示
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UpdateQCStatisticsForm_Load(object sender, EventArgs e)
        {
            InitUser();
        }
        ...
        private void BindCheckState()
        {
            ...
            //    cbxQCInfoState.SelectedIndex = 4;
            //}
        }
        /// <summary>
        /// 加载用户
        ...
        private void InitUser()
        {
            ...
            txtbeginTime.Value = DateTime.Now.AddMonths(-1);
            LoadData("");
            DateTimeBeginISNull();
            DateTimeEndIsNull();
        ...
        private void LoadData(string clickName)
        {
            try
            {
                expr = (Expression<Func<View_QCRecordInfo_Update, bool>>)PredicateExtensionses.True<View_QCRecordInfo_Update>();
                
                int i = 0;
                if (this.cbxQCInfoState.Text != "")//检测状态
                {

                    expr = expr.And(n => n.Dictionary_Name == cbxQCInfoState.Text);
                    i++;
                ...
                string beginTime = txtbeginTime.Value.ToString();
                string endTime = txtendTime.Value.ToString();
                string begin = beginTime;  // + " 00:00:00"
                string end = endTime;  // + "23:59:59 "
                if (beginTime != "")//开始时间
                {
                    expr = expr.And(n => n.QCRecord_UPDATE_TIME >= Common.GetDateTime(begin));
                    i++;
                }
                if (endTime != "")//结束时间   QCInfo表字段:QCIInfo_EndTime
                {
                    expr = expr.And(n => n.QCRecord_UPDATE_TIME <= Common.GetDateTime(end));
                    i++;
                }
                if (beginTime != "" && endTime != "")
                {
                    if (Common.GetDateTime(begin) > Common.GetDateTime(end))
                    {
                        ...
                        //expr = (Expression<Func<View_UpdateTongJI, bool>>)PredicateExtensionses.True<View_UpdateTongJI>();
                        return;
                    }
                    else
                    {
                        //expr = expr.And(n => n.QCRecord_UPDATE_TIME == Common.GetDateTime(begin) && n.QCRecord_UPDATE_TIME == Common.GetDateTime(end));
                        i++;
                    }
                ...
                dgvCarStatisticsQC.DataSource = page.BindBoundControl<View_QCRecordInfo_Update>(clickName, txtCurrentPage2, lblPageCount2, expr, orderbywhere);
        ...
        {
            string fileName = "理文检测系统修改质检数据统计Excel报表-" + DateTime.Now.ToShortDateString();
        ...
        {
            MenuInfoADD ma = new MenuInfoADD();
            ma.ShowDialog();
        }
