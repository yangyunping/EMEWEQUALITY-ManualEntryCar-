...
{
    public partial class UpdateQCDetailsForm : Form
    {
        // 设置公共的质检编号并初始化
        ...
        private void tsbDelete_Click()
        {
            try
            {
                ...
                if (this.dgv_SFJC.SelectedRows.Count > 0)//选中删除
                {
                    if (MessageBox.Show("确定要删除吗？", "系统提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        {
                            Expression<Func<QCRecord, bool>> funQCRecordinfo = n => n.QCInfo.QCInfo_ID == Convert.ToInt32(dgv_SFJC.SelectedRows[i].Cells["QCInfo_ID"].Value.ToString());
                        ...
                        string strContent = "质检编号为：" + Convert.ToInt32(dgv_SFJC.SelectedRows[0].Cells["QCInfo_ID"].Value.ToString()) + "，质检车牌号为：" + this.txtCarNO.Text.Trim() + "，删除成功！";
            ...
            catch (Exception ex)
            {
                ...
                Common.WriteTextLog("修改质检数据统计详情 tsbDelete_Click()+" + ex.Message.ToString());
            }
            finally
            {
                page = new PageControl();
        ...
        private void bdnInfo_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            dgv_SFJC.DataSource = page.BindBoundControl<View_QCRecordInfo>(e.ClickedItem.Name, txtCurrentPage2, lblPageCount2, expr);
        }
